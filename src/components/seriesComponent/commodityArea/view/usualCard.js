...
import {Card, Row, Col, Avatar, Button, Tooltip, List, Spin, Pagination, Divider, Icon} from 'antd';
...
class usualCard extends React.Component {
    ...
    constructor(props) {
        ...
        // this.props.onFetchCommodities(this.props.seriesId);
    }
    ...
    componentDidMount() {
        ...
        if (this.props.seriesId !== undefined) {
            this.props.onInit(this.props.seriesId.seriesId);
        }
    }
        ...
        return (
            <div>
                ...
                <Row>
                    {
                        // 遍历显示当前页面 商品数据
                        commList.map(function (item, index) {
                            return (
                                <Card // 商品卡片
                                    key={index}
                                    bordered={false}
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        onInit: (seriesId) => {
            dispatch(Actions.Start());
            dispatch(Actions.Init(seriesId));
        },
        // 收藏
